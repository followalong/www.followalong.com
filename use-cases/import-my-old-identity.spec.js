...
describe('Import my old identity', () => {
  ...
  beforeEach(async () => {
    ...
    // The file picker lives in a sheet now, like the rest of the You page.
    await app.click('[aria-label="Import legacy identity"]')
  ...
  test('shows both saved entries under Saved', async () => {
    // The sheet stays open on purpose, holding the report of what came in,
    // and the signal chips live on the river rather than on You.
    await app.click('[data-sheet-close]')
    await app.click('[aria-label="Home"]')
